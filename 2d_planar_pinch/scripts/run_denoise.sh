...
EOF
    ...
    if [[ "$GPU_SUPPORT" == "true" && "$NTASKS" -gt 1 ]]; then
        ...
        echo "srun --exclusive -N ${NNODES} -G ${total_gpus} -n ${NTASKS} $cmd 2>&1 | tee denoise.${PLATFORM}.log" >> "$jobfile"
    else
        echo "$cmd 2>&1 | tee denoise.${PLATFORM}.log" >> "$jobfile"
    fi
}
...
EOF
    ...
    if [[ "$GPU_SUPPORT" == "true" && "$NTASKS" -gt 1 ]]; then
        echo "flux run --exclusive --nodes=${NNODES} --ntasks ${NTASKS} $cmd 2>&1 | tee denoise.${PLATFORM}.log" >> "$jobfile"
    else
        echo "$cmd 2>&1 | tee denoise.${PLATFORM}.log" >> "$jobfile"
    fi
}
...
run_interactive() {
    ...
    if [[ "$ACTION" == "train" ]]; then
        if [[ -n "$OUT_DIR" ]]; then
            ...
            info "  Output dir: runs.$PLATFORM (default)"
        fi
    fi
    ...
    case "$scheduler" in
        slurm)
            ...
            local runcmd="srun --exclusive -p $debug_queue"
            $runcmd $cmd 2>&1 | tee "$logfile"
            ;;
        flux)
            local debug_queue=$(get_config "$PLATFORM" "debug_queue" "pdebug")
            local runcmd="flux run --exclusive --nodes=$NNODES -q=$debug_queue"
            $runcmd $cmd 2>&1 | tee "$logfile"
            ;;
        direct)
...
run_batch() {
    ...
    # Change to working directory for batch job
    cd "$WORKDIR"
