...
void ImplicitSolver::CumulateJ ( int from_nl_stage, int nl_iter )
{
...
for (int lev = 0; lev < m_num_amr_levels; ++lev) {
    ...
    for (int dir = 0; dir < 3; ++dir) {
        const amrex::MultiFab& mf = *J[dir];
        const amrex::MultiFab& E_mf = *E[dir];
        const amrex::MultiFab& E0_mf = *E0[dir];

        for (amrex::MFIter mfi(mf); mfi.isValid(); ++mfi) {
            amrex::Box box = amrex::convert(mfi.validbox(),J[dir]->ixType());
            box.grow(J[dir]->nGrowVect());

            auto const& arr = mf.array(mfi);
            auto const& E_arr = E_mf.array(mfi);
            auto const& E0_arr = E0_mf.array(mfi);

            const int ilo = box.smallEnd(0);
            const int ihi = box.bigEnd(0);

            for (int i = ilo; i <= ihi; ++i) {
                if (std::abs(arr(i,0,0,0)) > 0.) {
                    if (from_nl_stage && dir == 0) {
                amrex::Print() << "nl = " << nl_iter <<", Jx (i="<<i<<") = " << arr(i,0,0,0) << std::endl;
                amrex::Print() << "        Ex (i="<<i<<") = " << E_arr(i,0,0,0) << std::endl;
                    }
                }
            }
        }
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
        ...
        for ( amrex::MFIter mfi(*J[0], false); mfi.isValid(); ++mfi )
        {
            ...
            Jbx, ncomps, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n)
            {
                ...
                if (std::abs(Jx(i,j,k,n)) > 0.0) {
                    amrex::Print() << "ls    Jx (i="<<i<<") = " << Jx(i,j,k,n) << std::endl;
                    amrex::Print() << "ls    Jx0(i="<<i<<") = " << Jx0(i,j,k,n) << std::endl;
                    amrex::Print() << "ls    Ex (i="<<i<<") = " << Ex(i,j,k,n) << std::endl;
                    amrex::Print() << "ls    Ex0(i="<<i<<") = " << Ex0(i,j,k,n) << std::endl;
                }
            ...
            Jby, ncomps, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n)
            {
                ...
                if (std::abs(Jy(i,j,k,n)) > 0.0) {
                    amrex::Print() << "ls    Jy (i="<<i<<") = " << Jy(i,j,k,n) << std::endl;
                    amrex::Print() << "ls    Jy0(i="<<i<<") = " << Jy0(i,j,k,n) << std::endl;
                    amrex::Print() << "ls    Ey (i="<<i<<") = " << Ey(i,j,k,n) << std::endl;
                    amrex::Print() << "ls    Ey0(i="<<i<<") = " << Ey0(i,j,k,n) << std::endl;
                }
            ...
            Jbz, ncomps, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n)
            {
                ...
                if (std::abs(Jz(i,j,k,n)) > 0.0) {
                    amrex::Print() << "ls    Jz (i="<<i<<") = " << Jz(i,j,k,n) << std::endl;
                    amrex::Print() << "ls    Jz0(i="<<i<<") = " << Jz0(i,j,k,n) << std::endl;
                    amrex::Print() << "ls    Ez (i="<<i<<") = " << Ez(i,j,k,n) << std::endl;
                    amrex::Print() << "ls    Ez0(i="<<i<<") = " << Ez0(i,j,k,n) << std::endl;
                }
                                ...
                                const bool         a_from_jacobian )
{
    ...
    if (m_use_mass_matrices_jacobian && a_from_jacobian) { // Called from linear stage of JFNK and using mass matrices for Jacobian
        ...
        m_WarpX->PushParticlesandDeposit(a_cur_time, skip_deposition, PositionPushType::Full, MomentumPushType::Full, &options);
        CumulateJ(!a_from_jacobian, a_nl_iter);
    }
